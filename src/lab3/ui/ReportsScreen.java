...
public class ReportsScreen extends BaseScreen {
    ...
    public ReportsScreen(Stage stage) { super(stage); }

    public void show() {
        VBox content = new VBox(20);
        content.setStyle("-fx-background-color:transparent;");

        Label title = new Label("Transaction Reports");
        ...
        filterCard.setMaxWidth(900);

        Label filterTitle = new Label("Filter & Export");
        filterTitle.setStyle("-fx-font-size:16px;-fx-font-weight:bold;-fx-text-fill:white;-fx-font-family:'Times New Roman';");
        ...
        Button dailyBtn = new Button("Daily Summary CSV");
        dailyBtn.getStyleClass().add("btn-secondary");
        dailyBtn.setOnAction(e -> exportDailySummary());

        // Align buttons to bottom of their group
        VBox btnGroup = new VBox(5);
        Label spacer = new Label(" "); spacer.getStyleClass().add("field-label");
        HBox btnRow = new HBox(10);
        btnRow.getChildren().addAll(filterBtn, allBtn, exportBtn, dailyBtn);
        btnGroup.getChildren().addAll(spacer, btnRow);

        filterRow.getChildren().addAll(fromGroup, toGroup, btnGroup);
        HBox.setHgrow(btnGroup, Priority.ALWAYS);

        // Summary stats
        summaryLabel = new Label("Showing all transactions");
        summaryLabel.setStyle("-fx-font-size:13px;-fx-text-fill:#D4A017;-fx-font-family:'Times New Roman';");

        filterCard.getChildren().addAll(filterTitle, filterRow, summaryLabel);
        ...
        root.setTop(buildTopBar("Reports"));
        ...
        stage.setTitle("IgirePay - Reports");
        stage.show();
        loadAll();
    }
    ...
    private void applyFilter() {
        ...
        if (from == null || to == null) { summaryLabel.setText("Select both dates."); return; }
        if (from.isAfter(to))           { summaryLabel.setText("From date must be before To date."); return; }
        try {
            List<Transaction> all  = txSvc.getByCustomer(SessionManager.getUser().getCustomerId());
            List<Transaction> list = all.stream().filter(t ->
                t.getCreatedAt() != null &&
    ...
    private void exportCSV() {
        ...
        fc.setInitialFileName("igirepay_transactions_" + LocalDate.now() + ".csv");
    ...
    private void exportDailySummary() {
        ...
        fc.setInitialFileName("igirepay_daily_summary_" + LocalDate.now() + ".csv");
        fc.getExtensionFilters().add(new FileChooser.ExtensionFilter("CSV Files", "*.csv"));
        File file = fc.showSaveDialog(stage);
        if (file != null) {
            try {
